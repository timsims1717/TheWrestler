...
public class StickyFingersPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public StickyFingersPower(final AbstractCreature owner, final AbstractCreature source, final int amount) {
        name = NAME;
        ID = POWER_ID;
    ...
    public int onAttacked(DamageInfo info, int damageAmount) {
        if (info.type != DamageInfo.DamageType.THORNS && info.type != DamageInfo.DamageType.HP_LOSS && info.owner != null && info.owner != this.owner) {
            flash();
            addToTop(new ApplyPowerAction(info.owner, (AbstractPlayer) this.owner, new GrapplePower(info.owner, this.owner, amount), amount));
        }
        return damageAmount;
    }
    ...
    public AbstractPower makeCopy() {
        return new StickyFingersPower(owner, source, amount);
    }
}
